<div
    x-data="{
        useUrl: {{ isset($gallery) && str_starts_with($gallery->image ?? '', 'http') ? 'true' : 'false' }}
    }"
    class="space-y-4">
    ...
    <div x-show="!useUrl" x-transition class="space-y-2">
        ...
        @if(isset($gallery) && $gallery->image && !str_starts_with($gallery->image, 'http'))
        <img
            src="{{ asset('storage/' . $gallery->image) }}"
            class="mt-2 w-40 h-24 object-cover rounded-lg">
        @endif
    </div>
    ...
    <div x-show="useUrl" x-transition class="space-y-2">
        <input type="url"
            name="image_url"
            value="{{ isset($gallery) && str_starts_with($gallery->image ?? '', 'http') ? $gallery->image : '' }}"
            placeholder="Masukkan URL gambar..."
            class="w-full rounded-lg border border-gray-300 dark:border-gray-700 px-3 py-2 text-sm" />

        @if(isset($gallery) && str_starts_with($gallery->image ?? '', 'http'))
        <img
            src="{{ $gallery->image }}"
            class="mt-2 w-40 h-24 object-cover rounded-lg">
        @endif
