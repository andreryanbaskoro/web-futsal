    ...
    <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="overflow-hidden">
            ...
            : item.image"
                                ...
                                {{-- NAMA LAPANGAN --}}
                                <td class="px-4 py-4">
                                    <div class="font-medium text-gray-800 dark:text-white" x-text="item.nama_lapangan"></div>
                                </td>

                                {{-- DESKRIPSI --}}
                                <td class="px-4 py-4 text-sm text-gray-500 max-w-xs truncate" x-text="item.deskripsi"></td>

                                {{-- DIMENSI --}}
                                <td class="px-4 py-4 text-sm text-gray-600"><span x-text="item.dimensi ?? '-'"></span></td>

                                {{-- KAPASITAS --}}
                                <td class="px-4 py-4 text-sm text-gray-600"><span x-text="item.kapasitas ?? '-'"></span></td>
                                ...
                                <td class="px-4 py-4 text-sm text-gray-600">
                                    <div class="flex items-center gap-1">
                                        ...
                                        <span class="text-xs text-gray-400">(<span x-text="item.rating_count ?? 0"></span>)</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4">
                                    <span class="inline-flex rounded-full px-2 py-1 text-xs font-semibold"
                                        :class="item.status === 'aktif' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'"
                                        x-text="item.status">
                                    </span>
                                </td>
                                ...
                                <td class="px-4 py-4 text-sm text-gray-500">
                                    <div>Dibuat: <span x-text="new Date(item.created_at).toLocaleDateString('id-ID')"></span></div>
                                    <div x-show="item.created_at !== item.updated_at">Diperbarui: <span x-text="new Date(item.updated_at).toLocaleDateString('id-ID')"></span></div>
                                </td>

                                {{-- AKSI --}}
                                <td class="px-4 py-4 text-end">
                                    <div class="flex justify-end gap-3">
                                        <a :href="`/admin/lapangan/${item.id_lapangan}/edit`" class="text-blue-500 hover:text-blue-700">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form :action="`/admin/lapangan/${item.id_lapangan}`" method="POST" x-ref="lapanganDeleteForm" @submit.prevent="showDeleteModal($event)">
                                            @csrf
                                            @method('DELETE')
